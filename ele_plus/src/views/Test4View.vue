  ...
  <div id="field-graph" class="w-full h-full relative">
    ...
    >
      ...
      <div class="mt-2 space-y-1">
        ...
        ><br />
        <button @click="highlightTargets" class="text-blue-500 hover:underline">查看去向字段</button
        ><br />
        <button @click="jumpToFieldGraph" class="text-green-600 hover:underline">
          查看字段血缘图
        </button>
      </div>
    </div>
...
function highlightSources() {
  graphInstance.getEdges().forEach((e) => {
    ...
    graphInstance.setItemState(
      e,
      'highlight',
      model.target === tooltip.value.table && model.targetAnchor === tooltip.value.anchor,
    )
  })
}
...
function highlightTargets() {
  graphInstance.getEdges().forEach((e) => {
    ...
    graphInstance.setItemState(
      e,
      'highlight',
      model.source === tooltip.value.table && model.sourceAnchor === tooltip.value.anchor,
    )
  })
}
    ...
    return {
      ...
      sourceAnchor:
        tables
          ...
          ?.fields.findIndex((f) => f.name === sourceField) + 1,
      targetAnchor:
        tables
          .find((t) => t.name === targetTable)
          ?.fields.findIndex((f) => f.name === targetField) + 1,
    }
  })
