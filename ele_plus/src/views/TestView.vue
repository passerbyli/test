<template>
    <div id="field-graph" class="w-full h-full relative">
        <div v-if="selectedNode" class="absolute top-4 left-4 bg-white border rounded shadow p-4 z-10 max-w-md">1
...
onMounted(() => {
    ...
    graph.on('node:click', (evt) => {
        ...
        graph.getEdges().forEach((edge) => {
            ...
            graph.setItemState(edge, 'highlight', edgeModel.source === model.id || edgeModel.target === model.id);
        });
