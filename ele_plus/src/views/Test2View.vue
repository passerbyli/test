<template>11
    <div id="field-graph" class="w-full h-full"></div>22
</template>
...
onMounted(() => {
    ...
    const g6edges = edges.map(({ source, target }) => {
        return {
            ...
            sourceAnchor: tables.find(t => t.name === source.split('.')[0])?.fields.indexOf(source.split('.')[1]) + 1,
            targetAnchor: tables.find(t => t.name === target.split('.')[0])?.fields.indexOf(target.split('.')[1]) + 1,
        };
    });
