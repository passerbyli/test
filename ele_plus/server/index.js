...
const { getUserData, setUserDataJsonProperty } = require('./utils/storeUtil.js')
  ...
  if (options.enableAutoLogin && cookies && cookies.length > 0) {
    page
      .waitForResponse(async (response) => {
        ...
        if (responseUrl.startWith(API.get_user_info_url) && response.status() === 200) {
          const responseText = (await response.text()) || ''
          const responseJson = JSON.parse(
            responseText.substring(responseText.indexOf('(') + 1, responseText.lastIndexOf(')')),
          )
