...
function getTargetSheetXlsx(
  workbook,
  { sheetName, sheetIndex, useActiveSheet }
) {
  if (useActiveSheet && workbook.Workbook?.WBProps?.activeTab !== undefined) {
    return workbook.Sheets[
      workbook.SheetNames[workbook.Workbook.WBProps.activeTab]
    ];
  }
  if (sheetName) {
...
function getTargetSheetExceljs(
  workbook,
  { sheetName, sheetIndex, useActiveSheet }
) {
  if (useActiveSheet) {
...
function getTargetSheetXlsxPopulate(
  workbook,
  { sheetName, sheetIndex, useActiveSheet }
) {
  if (useActiveSheet) {
