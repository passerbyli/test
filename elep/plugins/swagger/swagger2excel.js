...
function schemaToType(doc, schema) {
  ...
  if (s.oneOf)
    return (
      "oneOf(" + s.oneOf.map((x) => schemaToType(doc, x)).join(" | ") + ")"
    );
  if (s.anyOf)
    return (
      "anyOf(" + s.anyOf.map((x) => schemaToType(doc, x)).join(" | ") + ")"
    );
  if (s.allOf)
    return (
      "allOf(" + s.allOf.map((x) => schemaToType(doc, x)).join(" & ") + ")"
    );
  if (s.type === "array")
    return `array<${schemaToType(doc, s.items) || "any"}>`;
...
function flattenProperties(doc, schema, parent = "", level = 1, rows = []) {
  ...
  for (const [name, prop0] of Object.entries(props)) {
    ...
    rows.push({
      ...
      simpleValue: "",
      possible:
        (Array.isArray(prop.enum) && prop.enum.map(String).join(",")) || "",
    });
...
function collectParameters(doc, pathItem, op) {
  const list = [
    ...arrayify(pathItem.parameters),
    ...arrayify(op.parameters),
  ].filter(Boolean);
  return list.map((p) => ({
    In: p.in || "",
    Name: p.name || "",
...
function buildWorkbook(indexRows, apiSheets) {
  ...
  for (const api of apiSheets) {
    ...
    api.Request.forEach((x) => {
      const lvl = Number.isFinite(Number(x.lv))
        ? Math.max(1, Math.floor(Number(x.lv)))
        : 1;
...
(function main() {
  ...
  wb.write(OUTPUT, (err) => {
    ...
    console.log(
      `   接口数量：${apiSheets.length}，首页清单行数：${indexRows.length}`
    );
  });
})();
