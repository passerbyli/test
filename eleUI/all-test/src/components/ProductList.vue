<template>
    <div style="padding: 10px">
        ...
        <el-row style="margin: 5px 0">
            ...
            <el-col :span="1">
                <el-button size="small" type="primary" @click="handleAddAllToShoppingList">添加该页所有商品到抢购列表
                </el-button>
            </el-col>
        </el-row>
            ...
            <el-table-column prop="productName" label="名称" width="350">
                <template #default="scope">
                    <el-tooltip content="在浏览器中查看" placement="top" :hide-after="0">
                        <span class="product-link" @click="handleOpenLink(scope.row.id)">{{ scope.row.productName
                        }}</span>
                    </el-tooltip>
                </template>
            </el-table-column>
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const dataMap = reactive({
            ...
            imageUrl: computed(
                () => (primaryPic, size) => {
                    API.image_url + size + "/" + (primaryPic.startsWith("jfs") ? primaryPic : "jfs/" + primaryPic)
                }
            ),
            productSearchResult: {
                pageCount: 0,
