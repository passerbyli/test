...
;(async () => {
  ...
  const INSTALLER = process.env.INSTALLER || 'npm' // 可选值：npm、yarn、pnpm
  ...
  function installCommand(installer) {
    if (installer === 'yarn') return 'yarn install'
    if (installer === 'pnpm') return 'pnpm install'
    ...
    try {
      ...
      const tasks = projectPaths.map((projectPath) =>
        limit(async () => {
          ...
          let installed = false
          try {
            execSync(installCommand(INSTALLER), { cwd: projectPath, stdio: 'ignore' })
            installed = true
          } catch {
            output.push({
              仓库名: repoName,
